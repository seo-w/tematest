<form class="property-search-form" action="<?php if ( tt_page_id_template_search() ) { echo get_permalink( tt_page_id_template_search() ); } ?>">
			...
			foreach ( $search_fields as $search_field ) {
				...
				if ( ! empty( $search_field ) ) {
					...
					if ( in_array( $search_field, $default_search_fields_array ) ) {
						...
						switch ( $search_field ) {
							...
							<div class="<?php echo $columns; ?> form-group select">	
                                ...
                                <?php if(!empty($search_labels[$i])){
									    ...
									    $search_label_status = $search_labels[$i];
										 ...
										 $search_label_status = 'Any Status';
										 
									  }
								?>
								<select name="<?php echo $search_parameter; ?>" id="<?php echo $search_parameter; ?>" class="form-control">
									<option value="all"><?php _e( $search_label_status , 'tt' ); ?></option>
							    <?php 
							...
							<div class="<?php echo $columns; ?> form-group select">	
                                ...
                                <?php if(!empty($search_labels[$i])){
									    ...
									    $search_label_type = $search_labels[$i];
										 ...
										 $search_label_type = 'Any Type';
										 
									  }
								?>
								<select name="<?php echo $search_parameter; ?>" id="<?php echo $search_parameter; ?>" class="form-control">
									<option value="all"><?php _e( $search_label_type , 'tt' ); ?></option>
							    <?php 
							...
							<div class="<?php echo $columns; ?> form-group price-range">
								...
								<label><?php echo __( $search_labels[$i], 'tt' ); ?> <span class="price-range-min" id="price-range-min"></span> <?php _e( 'to', 'tt' ); ?> <span class="price-range-max" id="price-range-max"></span></label>
					...
					//}
						...
						if ( $acf_field_type == 'select' || $acf_field_type == 'checkbox' || $acf_field_type == 'radio' || $acf_field_type == 'taxonomy' ) {
	...
	<?php 
	$default_order = $realty_theme_option['search_results_default_order'];
	?>
	<input type="hidden" name="order-by" value="<?php echo $default_order; ?>" />
...
if($acf_field_type == 'taxonomy'){ 
							...
							?>
								...
								?>
								<select name="<?php echo $acf_field['name']; ?>" id="<?php echo $acf_field['name']; ?>" class="form-control">
									<option value="all"><?php _e( $search_label_type , 'tt' ); ?></option>
							    <?php 
								
								print_r($acf_field);
							    $acf_type = get_terms($acf_field['taxonomy'], array( 'orderby' => 'slug', 'parent' => 0 ) ); 
							    if ( isset( $_GET[$search_parameter] ) ) {
										$acf_get_type = $_GET[$search_parameter];
									}
									else {
										$acf_get_type = '';
									}
									?>
							    <?php foreach ( $acf_type as $acf_key => $acf_type ) : ?>
					        <option value="<?php echo $acf_type->slug; ?>" <?php selected( $acf_type->slug, $acf_get_type ); ?>>
				            <?php 
				            echo $acf_type->name;
				            $acf_type2 = get_terms( $acf_field['taxonomy'] , array( 'orderby' => 'slug', 'parent' => $acf_type->term_id ) );
				            if( $acf_type2 ) : 
				            ?>
				            <optgroup>
				              <?php foreach( $acf_type2 as $acf_key => $acf_type2 ) : ?>
				                  <option value="<?php echo $type2->slug; ?>" class="level2" <?php selected( $acf_type2->slug, $acf_get_type ); ?>>
				                  	<?php 
				                  	echo $acf_type2->name;
				                  	$acf_type3 = get_terms( $acf_field['taxonomy'] , array( 'orderby' => 'slug', 'parent' => $acf_type2->term_id ) );
				                  	if( $acf_type3 ) : ?>
				                  	<optgroup>
				                  		<?php foreach( $acf_type3 as $acf_key => $acf_type3 ) : ?>
				                    		<option value="<?php echo $acf_type3->slug; ?>" class="level3" <?php selected( $acf_type3->slug, $acf_get_type ); ?>>
				                    		<?php 
				                    		echo $acf_type3->name;
					                    	$acf_type4 = get_terms( $acf_field['taxonomy'] , array( 'orderby' => 'slug', 'parent' => $acf_type3->term_id ) );
					                    	if( $acf_type4 ) :
				                    		?>
				                    		<optgroup>
				                    			<?php foreach( $acf_type4 as $acf_key => $acf_type4 ) : ?>
				                    			<option value="<?php echo $acf_type4->slug; ?>" class="level4" <?php selected( $acf_type4->slug, $acf_get_type ); ?>>
																	<?php echo $acf_type4->name; ?>
				                    			</option>
				                    			<?php endforeach; ?>
				                    		</optgroup>
				                    		<?php endif; ?>
