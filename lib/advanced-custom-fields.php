...
if ( ! function_exists('tt_acf_group_id_property') ) {
	function tt_acf_group_id_property() {
	    ...
	    $group_id = array();
		$the_query = new WP_Query( array( 'post_type' => array('acf-field','acf-field-group','acf'), 'posts_per_page' => -1,'post_status'=>'publish', 'location'=>array(array('param'=>'post_type', 'operator'=>'==','value'=>'property')) ) );
...
/* ACF CSV import helper functios. */
...
function tt_csv_data_import_acf(){
	if ( class_exists( 'acf' ) ) {
		...
		add_action( 'admin_notices', 'acf_admin_error_notice_csv' ); 
	}
}
}
