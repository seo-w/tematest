...
function tt_property_search_args($search_results_args) {
	...
	foreach ( $_GET as $search_key => $search_value ) { 
		...
		// Check If Key Has A Value
		if ( ( ! empty( $search_value ) || $search_key == "price_range_min" ) && $search_key != "order-by" && $search_key != "pagenumber" && $search_key != "pageid" && $search_key!="searchid" ) {
			// Search Form Mini
...
function tt_ajax_search() {
	...
	<ul class="row list-unstyled">	
		...
		if ( empty($columns ) || $realty_theme_option['property-search-results-template']!='default-template' ) {
			$columns = "col-md-6";
		}
		?>
