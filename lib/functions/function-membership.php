...
if ( ! function_exists( 'tt_create_user_invoice' ) ) {
	...
	function tt_create_user_invoice( $item_id, $method ) {
		...
		if ( $user_id && $item_id && $current_user_role == 'subscriber' ) {
			...
			if ( get_post_type( $item_id ) == 'package' ) {
				...
				//$from_date=date('d/m/Y', strtotime($time_now));
...
if ( ! function_exists( 'tt_update_property_payment_data' ) ) {
	...
	function tt_update_property_payment_data( $invoice_id, $property_id, $first_name, $last_name, $email_user ) {
		...
		update_post_meta( $property_id, 'property_payment_status', __( 'Completed', 'tt' ) );
		...
		if ( $query_properties->have_posts() ) :	
			...
			while ( $query_properties->have_posts() ) : $query_properties->the_post();
					...
					if ( ! empty( $payment_date ) ) {
						...
						if ( $is_valid == false ) {
							update_post_meta( $post->ID, 'property_payment_status', __( 'Expired', 'tt' ), __( 'Completed', 'tt' ) );
							$property = array(
								'ID'		  		=> $post->ID,
								'post_status' => 'pending',
							);
...
if ( ! function_exists( 'tt_package_expiry_email' ) ) {
	...
	function tt_package_expiry_email( $user_id, $package_id ) {
		...
		$body .= 'Your subscribed membership pakcage <strong>' . get_the_title( $package_id ) . '</strong> is expired, status for all of your properties is changed from Published to "Pending". Please update your package so that your properties are published again.</br></br>';
