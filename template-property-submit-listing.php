...
<div class="container">
	...
	if ( is_user_logged_in() ) {
		...
		// In case Strip payment failed
		if ( isset( $_GET['payment'] ) && $_GET['payment'] == 'failed' ) {
			echo '<p class="alert alert-danger">' . __( 'Payment failed! Please try again with correct information.', 'tt' ) . '</p>';
		}
		if ( isset( $_GET['payment'] ) && $_GET['payment'] == 'paid' ) {
			echo '<p class="alert alert-info">' . __( 'Payment completed! Thank you.', 'tt' ) . '</p>';
		}
